...
const ensureAllDefinedPathParamsAreUsedInPath = (
  ...
  for (const p of Object.keys(params)) {
    ...
    if (!(expected.includes(p) || expected.includes(p + '+'))) {
...
export const ycPathParam: IFunction = targetVal => {
  /**
   * This rule verifies:
   *
  ...
  const validOperationKeys = ['get', 'head', 'post', 'put', 'patch', 'delete', 'options', 'trace', 'x-yc-apigateway-any-method'];
